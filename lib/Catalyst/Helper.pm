...
sub mk_component {
    ...
    unless ( $_[0] =~ /^(?:model|view|controller|action)$/i ) {
        my $helper = shift;
        my @args   = @_;
        ...
        my $type   = shift;
        my $name   = shift || "Missing name for model/view/controller/action";
        my $helper = shift;
        my @args   = @_;
        return 0 if $name =~ /[^\w\:]/;
        $type              = lc $type;
        ...
        $self->{test} = $self->next_test unless $type eq 'Action';
        ...
        # Fallback
            ...
            $self->_mk_comptest unless $type eq 'Action';
        }
    }
    return 1;
}
...
[% appprefix %]_create.pl [options] model|view|controller name [helper] [options]
 ...
 Examples:
   ...
   [% appprefix %]_create.pl action Test
...
use strict;
use warnings;
[% IF long_type == 'Action' %]use Moose::Role;
[% ELSE %]
use base 'Catalyst::[% long_type %]';
[% END %]
=head1 NAME

[% class %] - [% app %] [% name %] [% long_type %]
